          ...
          <article className="rounded-[2rem] border border-rose-200 bg-rose-50/90 p-6 shadow-soft sm:p-7">
            ...
            <ul className="mt-6 space-y-3">
              {beforeItems.map((item) => (
                <li key={item} className="rounded-[1.25rem] border border-rose-200 bg-white/95 px-5 py-4 text-sm font-medium text-rose-700 shadow-soft">
                  {item}
                </li>
              ))}
            </ul>
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-3">
          {visualPillars.map((pillar, index) => (
            ...
            >
              ...
              <p className="mt-3 text-sm leading-relaxed text-slate-600 sm:text-base">{pillar.text}</p>
            </article>
          ))}
        </div>
